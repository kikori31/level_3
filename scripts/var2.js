...
item1_total = item1+(item1_num+1)+'장의 가격은'+item1_price*2+'원 입니다.'; // 우선적으로 계산하게 해줄 때'()' 삽입
console.log(item1_total);
...
function numValP() { // 증가
    ...
    //frmNum.value+1 실행 시 1+1 = 11 나오는 오류 원인 찾기
    //typeof(frmNum.value) 함수실행 시 string 문자로 인식하는 원인발견
...
//함수 활용 성인, 어린이 +,- 버튼 클릭 시 성인수,어린이수를 증가/감소하는 함수 제작
...
function adultNumPlus() {
    ...
    // adultNum.textContent+childNum.textContent //어린이span태그의 내용 + 성인태그의 내용
    ...
    console.log(adultNum.textContent);
}
// 성인 감소 함수
...
function childNumPlus() {
    ...
    totalFunc();
    console.log(childNum.textContent);
}
// 어린이 감소 함수
