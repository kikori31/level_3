...
* `객체.속성 = '값';`
    ...
    * `<span>0</span>` //span의 내용 0을 -> 1로 변경
        * `span.textContent = 1;` // span원래내용은 제거하고 1 새로 대입 
        * `span.textContent = span.textContent+1;` //기존 내용에 +1 대입
    * `<input type="text" value="1">` // input의 내용 1 -> 2로 변경
        * `input.value = 2;`// input원래내용1을 제거하고 새로 2 대입
        * `input.value = input.value+2;` // 기존input의값에 +2를 대입
* `객체.함수();`
    * 객체를 실행한다
    * `객체.함수() = '값';` (x)
* `이벤트="함수();"`
    * 이벤트 동작 시 특정 함수를 실행한다.
    * `<태그 onclick="finc1();">` //함수호출
...
3. 2번 변수에 대입연산자로 객체 대입하기
    * `documnent.querySelector('HTML대상');`
...
## 실제 요소 크기와 관계없는 다양한 다지인 포인트 만드는 방법
